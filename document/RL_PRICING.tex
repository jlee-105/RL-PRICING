...
In a modern fab ramp-up, approximately 2{,}000 tools must pass through this pipeline.
...
\subsection{Generalization to Larger Problem Sizes}\label{sec:generalization}

A central practical question is whether a policy trained on small instances remains useful at scale without retraining.
We train a single policy once on instances with $|\calJ|=10$ tasks and evaluate it, with no further training, on held-out instances of increasing size ($|\calJ| \in \{10, 20, 40, 80\}$), holding $|\calR|=2$ resources, $|\calM|=3$ parts, and $|\calQ|=3$ modes fixed across sizes so the GNN's input dimensionality matches between training and evaluation.
At each size we report three configurations---CG+CP-SAT (baseline), CG+RL with CP-SAT fallback, and CG+RL (pure, no fallback)---so that any degradation with scale can be attributed either to a widening objective gap or to a growing reliance on the CP-SAT fallback (\emph{fallback rate}: the fraction of CG iterations at which the RL pricer failed to find an improving column and CP-SAT was invoked).
Table~\ref{tab:generalization} and Figure~\ref{fig:generalization} report results averaged over 20 held-out instances per size.

\begin{table}[h]
\centering
\caption{Zero-shot generalization: a single policy trained on $|\calJ|=10$ is evaluated, without retraining, on increasing problem sizes (20 held-out instances per size, mean reported). \textbf{[PLACEHOLDER --- pending experiment]}}
\label{tab:generalization}
\begin{tabular}{llccccc}
\toprule
$|\calJ|$ & Method & Obj.\ Gap vs.\ CG+CP-SAT (\%) & CG Wall-clock (s) & Speedup ($\times$) & Fallback Rate (\%) & Columns \\
\midrule
\multirow{3}{*}{10 (train)}
  & CG+CP-SAT      & 0.00 & -- & 1.0$\times$ & n/a & -- \\
  & CG+RL+fallback & --   & -- & --          & --  & -- \\
  & CG+RL (pure)   & --   & -- & --          & n/a & -- \\
\midrule
\multirow{3}{*}{20}
...
\begin{figure}[h]
\centering
% Placeholder for actual generalization plots (3-panel)
\begin{minipage}{0.32\textwidth}
\centering
\fbox{\parbox{0.9\linewidth}{\centering\vspace{1.4cm}
\textit{(a) Obj.\ gap (\%) vs.\ $|\calJ|$}\\
\textit{RL pure vs.\ RL+fallback}
\vspace{1.4cm}}}
\end{minipage}\hfill
\begin{minipage}{0.32\textwidth}
\centering
\fbox{\parbox{0.9\linewidth}{\centering\vspace{1.4cm}
\textit{(b) Speedup ($\times$, log)}\\
\textit{vs.\ $|\calJ|$}
\vspace{1.4cm}}}
\end{minipage}\hfill
\begin{minipage}{0.32\textwidth}
\centering
\fbox{\parbox{0.9\linewidth}{\centering\vspace{1.4cm}
\textit{(c) Fallback rate (\%)}\\
\textit{vs.\ $|\calJ|$}
\vspace{1.4cm}}}
\end{minipage}
\caption{Generalization to larger problem sizes for a single policy trained once at $|\calJ|=10$ (vertical dashed line marks the training size in each panel; points to its right are zero-shot extrapolation). \textbf{[PLACEHOLDER --- pending experiment]}}
...
% TODO once Table/Figure~\ref{tab:generalization}/\ref{fig:generalization} are filled in with real numbers:
% trim or remove this paragraph so it doesn't contradict Section~\ref{sec:generalization}'s empirical results.
Section~\ref{sec:generalization} evaluates generalization to larger instances (20--80 tasks) without retraining.
Extending this to more complex precedence structures and instance sizes beyond 80 tasks remains an important next step.
...
\bibliography{rl_pricing_references}
